...
# Na samym końcu sprawdzane jest czy da się wjechać do miasta początkowego z ostantiego na ścieżce inna bramą niż rozpoczęto tak aby dopełnić cykl
...
def search(graph,gate, v, visited, path):
    if len(path) < len(graph):
        for w in reversed(gate): #odwrocenie jest tylko dlatego że daje lepsze efekty na testach
            ...
            if not visited[w]:
                visited[w] = True
                path.append(w)
                val=search(graph, out,w, visited, path)
...
def droga( G ):
    ...
    if l[n-1] not in (G[0])[1]: return None
    return l
